...
from encoding_custom.datasets.transforms import build_inst_target_transform, \
    build_transforms, build_inst_seg_target_transform
...
class BaseDataset(data.Dataset):
    ...
    def __init__(self, root, split, cfg, mode=None, crop_size=None,
        ...
        self.inst_target_transform = None
        if 'detect' in self.tasks:
            self.inst_target_transform = build_inst_target_transform(
                cfg, self.crop_size, **kwargs)
        if 'inst_seg' in self.tasks:
            self.inst_seg_target_transform = build_inst_seg_target_transform(
                cfg, self.crop_size, **kwargs)
