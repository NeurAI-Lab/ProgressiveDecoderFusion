...
class CityscapesLoader(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        bboxes, labels, inst_masks, inst_depths, box_ids = self.inst_tar(
        ...
        targets = OrderedDict(
            ...
            mask=mask, depth=depth, inst_depths=inst_depths,
            inst_masks=inst_masks, sem_cont=sem_cont)
        if self.transform:
            image, targets = self.transform(image, **targets)

        if 'inst_seg' in self.tasks:
            inst_masks = self.inst_seg_target_transform(
                targets['bboxes'], targets['inst_masks'])

        if self.inst_target_transform:
            bboxes, _ = self.inst_target_transform(
                bboxes=targets['bboxes'], labels=targets['labels'],
                inst_depths=targets['inst_depths'], inst_masks=inst_masks)
        ...
        targets = {'detect': bboxes, 'segment': mask,
                   'depth': targets['depth'], 'sem_cont': targets['sem_cont']}
