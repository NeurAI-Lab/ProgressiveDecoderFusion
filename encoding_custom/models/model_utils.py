...
from ..encoders_decoders.uninet_encoder import uninet_encoder
...
from ..heads.autoencoder import AutoEncoder, AutoEncoderHead


def get_encoder(encoder_type, norm_layer, backbone_feat_channels,
                segment_or_depth, cfg):
    models = {'resnet': uninet_encoder}
...
def get_decoder(decoder_type, norm_layer, en_feat_channels, cfg):
    ...
    uni_decoder = Decoder(cfg, de_in_channels, norm_layer,
...
def get_autoencoder_head(cfg, num_en_features, **kwargs):
    if cfg.MODEL.AE.DENSE_NAME == 'complete':
        return AutoEncoder(cfg, num_en_features, **kwargs)
    if cfg.MODEL.AE.DENSE_NAME == 'only_head':
