...
from encoding_custom.losses.aux_losses import extract_surface_normals, \
...
class SemanticContourEvaluator(BaseEvaluator):
    ...
    def __init__(self, cfg):
        ...
        self.sem_cont_ce_loss = SmoothedValue()

    def reset(self):
        pass

    def process(self, targets, predictions, image_idxs):
        consistency_loss = self.ce_loss(
            predictions['sem_cont'], predictions)
        self.sem_consist.update(consistency_loss)
        ce_loss = self.ce_loss(predictions['sem_cont'], targets)
        self.sem_cont_ce_loss.update(ce_loss)

    def reduce_from_all_processes(self):
        pass

    def evaluate(self):
        return {'key_metrics/sem_cont_ce_loss': self.sem_cont_ce_loss.global_avg,
                'metrics/semantic_consistency': self.sem_consist.global_avg}
