...
class BatchCollator(object):
    ...
    def __call__(self, batch):
        ...
        for task in tars[0].keys():
            ...
            if task == 'detect':
                # TODO: if inst seg mask is list of masks, imagelist is required...
                targets.update({task: ts})
            # TODO: this is not a task... hacky way.
            elif task == 'intrinsics' or task == 'classify':
                targets.update({task: default_collate(ts)})
            else:
                # TODO: pad value should be ignore_index..
...
def load_model(args, model, root_dir):
    start_epoch = 1
    ...
    if args.resume:
        ...
        replaced_ckpt = copy.deepcopy(checkpoint)

        model.load_state_dict(replaced_ckpt['model'])
...
def log_and_dump_metrics(val_metric_logger, path="./", epoch=-1,
    ...
    for meter_name, value in val_metric_logger.meters.items():
        if 'key_metrics/' in meter_name or 'metrics/' in meter_name:
            results.update({meter_name: value.global_avg})
...
class ModelSaver:
    ...
    def __init__(self, save_root, tasks, task_to_min_or_max, base_name='model',
